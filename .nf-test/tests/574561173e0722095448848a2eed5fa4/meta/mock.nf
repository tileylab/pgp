...
include { GATK4_HAPLOTYPECALLER } from '/Users/georgetiley/github/pgp/modules/local/gatk4/haplotypecaller/tests/../main.nf'
...
workflow {
    ...
    //run process
    GATK4_HAPLOTYPECALLER.run(input.toArray())

    if (GATK4_HAPLOTYPECALLER.output){

        // consumes all named output channels and stores items in a json file
        GATK4_HAPLOTYPECALLER.out.getNames().each { name ->
            serializeChannel(name, GATK4_HAPLOTYPECALLER.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = GATK4_HAPLOTYPECALLER.out as List<Object>
        def i = 0
        array.each { output ->
