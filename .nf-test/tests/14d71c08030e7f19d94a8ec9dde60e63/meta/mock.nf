...
include { GATK4_GENOMICSDBIMPORT } from '/Users/georgetiley/github/pgp/modules/local/gatk4/genomicsdbimport/tests/../main.nf'
...
workflow {
    ...
    //run process
    GATK4_GENOMICSDBIMPORT.run(input.toArray())

    if (GATK4_GENOMICSDBIMPORT.output){

        // consumes all named output channels and stores items in a json file
        GATK4_GENOMICSDBIMPORT.out.getNames().each { name ->
            serializeChannel(name, GATK4_GENOMICSDBIMPORT.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = GATK4_GENOMICSDBIMPORT.out as List<Object>
        def i = 0
        array.each { output ->
