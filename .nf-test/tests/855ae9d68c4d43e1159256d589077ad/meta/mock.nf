...
include { GATK4_SELECTVARIANTS } from '/Users/georgetiley/github/pgp/modules/local/gatk4/selectvariants/tests/../main.nf'
...
workflow {
    ...
    input = []
                ...
                // Local module takes 3-tuple: [meta, vcf, vcf_idx] — no intervals
                input[0] = [
                    [ id:'test' ],
    ...
    //run process
    GATK4_SELECTVARIANTS.run(input.toArray())

    if (GATK4_SELECTVARIANTS.output){

        // consumes all named output channels and stores items in a json file
        GATK4_SELECTVARIANTS.out.getNames().each { name ->
            serializeChannel(name, GATK4_SELECTVARIANTS.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = GATK4_SELECTVARIANTS.out as List<Object>
        def i = 0
        array.each { output ->
