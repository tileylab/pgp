...
include { FASTP } from '/Users/georgetiley/github/pgp/modules/local/fastp/tests/../main.nf'
...
workflow {
    ...
    input = []
                ...
                input[1] = file(params.modules_testdata_base_path + 'delete_me/fastp/adapters.fasta', checkIfExists: true)
                input[2] = false
                input[3] = false
                input[4] = true
    ...
    //run process
    FASTP.run(input.toArray())

    if (FASTP.output){

        // consumes all named output channels and stores items in a json file
        FASTP.out.getNames().each { name ->
            serializeChannel(name, FASTP.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = FASTP.out as List<Object>
        def i = 0
        array.each { output ->
