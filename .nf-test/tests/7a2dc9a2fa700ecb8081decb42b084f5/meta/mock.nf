...
include { GATK4_GENOTYPEGVCFS } from '/Users/georgetiley/github/pgp/modules/local/gatk4/genotypegvcfs/tests/../main.nf'
...
workflow {
    ...
    //run process
    GATK4_GENOTYPEGVCFS.run(input.toArray())

    if (GATK4_GENOTYPEGVCFS.output){

        // consumes all named output channels and stores items in a json file
        GATK4_GENOTYPEGVCFS.out.getNames().each { name ->
            serializeChannel(name, GATK4_GENOTYPEGVCFS.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = GATK4_GENOTYPEGVCFS.out as List<Object>
        def i = 0
        array.each { output ->
